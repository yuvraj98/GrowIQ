...
function MetricPill({ label, value, icon: Icon, color = 'text-gray-700 dark:text-gray-300' }) {
    return (
            ...
            <div className="p-6 flex-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    {[
                        ...
                        <div key={k.label} className="p-4 rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02] shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                ...
                <div className="flex items-center gap-3 mb-5 flex-wrap">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 rounded-lg bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 text-sm"
                        />
                    </div>
                    {['meta', 'google_ads'].map(p => (
                        <button key={p} onClick={() => setPlatform(prev => prev === p ? '' : p)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${platform === p ? PLATFORM_META[p].bg + ' ' + PLATFORM_META[p].color + ' ' + PLATFORM_META[p].border : 'border-gray-200 dark:border-white/5 text-gray-500 hover:border-gray-200 dark:hover:border-white/10 hover:text-gray-700 dark:text-gray-300'}`}>
                            {PLATFORM_META[p].label}
                        </button>
                    ))}
                    {['active', 'paused'].map(s => (
                        <button key={s} onClick={() => setStatus(prev => prev === s ? '' : s)}
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all capitalize ${status === s ? STATUS_STYLES[s] : 'border-gray-200 dark:border-white/5 text-gray-500 hover:border-gray-200 dark:hover:border-white/10 hover:text-gray-700 dark:text-gray-300'}`}>
                            {s}
                        </button>
                    ))}
                </div>
                ...
                {loading ? (
                    <div className="space-y-3">
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="h-20 rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02] animate-pulse" />
                        ))}
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="text-center py-20">
                        <BarChart3 className="w-12 h-12 text-gray-700 mx-auto mb-4" />
                        <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">No campaigns yet</h3>
                    ...
                    <div className="rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02] overflow-hidden shadow-sm">
                        {/* Table header */}
                        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_80px] gap-4 px-5 py-3 border-b border-gray-200 dark:border-white/5 text-xs text-gray-500 uppercase tracking-wide">
                            <span>Campaign</span>
                            <span>Spend (7d)</span>
                        ...
                        {filtered.map((c, idx) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_80px] gap-4 px-5 py-4 items-center hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-all cursor-pointer group ${idx !== filtered.length - 1 ? 'border-b border-gray-100 dark:border-white/5' : ''}`}
                                >
                                    {/* Name + meta */}
                                    ...
                                    <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">{formatCurrency(parseFloat(c.total_spend || 0))}</span>
                                    <span className="text-sm text-gray-700 dark:text-gray-300">{parseInt(c.total_impressions || 0).toLocaleString()}</span>
                                    <span className="text-sm text-gray-700 dark:text-gray-300">{parseInt(c.total_clicks || 0).toLocaleString()}</span>
                                    <span className="text-sm text-gray-700 dark:text-gray-300">{parseInt(c.total_conversions || 0).toLocaleString()}</span>
                                    <span className={`text-sm font-semibold ${roasColor}`}>{roas.toFixed(2)}x</span>
                                    <button
                                        onClick={e => { e.stopPropagation(); handleSync(c.id); }}
                                        className={`p-1.5 rounded-lg border border-gray-200 dark:border-white/5 text-gray-500 hover:text-violet-400 hover:border-violet-500/30 transition-all ${syncing === c.id ? 'animate-spin text-violet-400' : ''}`}
                                        title="Sync metrics"
                                    >
                    ...
                    <div className="flex items-center justify-between mt-5">
                        ...
                        <div className="flex gap-2">
                            <button disabled={pagination.page <= 1}
                                ...
                                className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 disabled:opacity-30">
                                Previous
                            </button>
...
function AddCampaignModal({ onClose, onCreated }) {
    ...
    const inputCls = "w-full px-3 py-2.5 rounded-lg bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 text-sm";
    const labelCls = "block text-xs text-gray-600 dark:text-gray-400 mb-1 font-medium";

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-gray-900/40 dark:bg-black/60 backdrop-blur-sm" onClick={onClose} />
            <div className="relative w-full max-w-lg rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 p-6 shadow-2xl max-h-[90vh] overflow-y-auto animate-fade-in-up">
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">Add Campaign</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <select required value={form.client_id} onChange={e => setForm({ ...form, client_id: e.target.value })} className={inputCls}>
                            <option value="" className="bg-white dark:bg-gray-900">Select client...</option>
                            {clients.map(c => <option key={c.id} value={c.id} className="bg-white dark:bg-gray-900">{c.business_name}</option>)}
                        </select>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select required value={form.platform} onChange={e => setForm({ ...form, platform: e.target.value })} className={inputCls}>
                                <option value="meta" className="bg-white dark:bg-gray-900">Meta Ads</option>
                                <option value="google_ads" className="bg-white dark:bg-gray-900">Google Ads</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 text-sm hover:bg-gray-100 dark:hover:bg-white/5 transition-all">Cancel</button>
