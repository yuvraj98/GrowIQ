        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    <div className="p-4 rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Total Reach</p>
                        <div className="flex items-end gap-2">
                            <span className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">24.8K</span>
                    ...
                    <div className="p-4 rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                        ...
                        <div className="flex items-end gap-2">
                            <span className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">4.2%</span>
                    ...
                    <div className="p-4 rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                        ...
                        <div className="flex items-end gap-2">
                            <span className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{posts.filter(p => p.status === 'scheduled').length}</span>
                        </div>
                    </div>
                    <div className="p-4 rounded-2xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02]">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-2 p-1 rounded-xl bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                        <button 
                            onClick={() => setSelectedClient('')}
                            className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${!selectedClient ? 'bg-white/10 text-slate-900 dark:text-white' : 'text-gray-500 hover:text-slate-900 dark:text-white'}`}
                        >
                            All Posts
                        </button>
                        {clients.map(c => (
                            <button 
                                ...
                                className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedClient === c.id ? 'bg-white/10 text-slate-900 dark:text-white' : 'text-gray-500 hover:text-slate-900 dark:text-white'}`}
                            >
                                {c.business_name}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {loading ? (
                        ...
                        <div className="lg:col-span-3 text-center py-24 rounded-3xl border border-dashed border-gray-200 dark:border-white/10">
                            <Calendar className="w-12 h-12 text-gray-700 mx-auto mb-4" />
                            <p className="text-gray-500">No content scheduled yet.</p>
                        </div>
                    ) : (
                        posts.map(post => (
                            <div key={post.id} className="group flex flex-col rounded-3xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/[0.02] shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:border-violet-500/20 dark:hover:border-white/10 transition-all duration-200 overflow-hidden">
                                {/* Post Body */}
                                <div className="p-5 flex-1">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-2">
                                            <div className="p-2 rounded-xl bg-white/5 text-slate-900 dark:text-white">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-1.5 mt-0.5">
                                                    ...
                                                    <span className="text-[10px] text-gray-600 dark:text-gray-400 capitalize">{post.status}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="p-4 rounded-2xl bg-white/5 border border-gray-200 dark:border-white/5 mb-4">
                                        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed italic">"{post.content}"</p>
                                    </div>

                                    {post.status === 'published' && (
                                        <div className="grid grid-cols-3 gap-2">
                                            <div className="p-2 rounded-xl bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 text-center">
                                                <p className="text-[8px] text-gray-500 font-bold uppercase mb-0.5">Likes</p>
                                                <p className="text-xs font-bold text-slate-900 dark:text-white">{post.likes || 0}</p>
                                            </div>
                                            <div className="p-2 rounded-xl bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 text-center">
                                                <p className="text-[8px] text-gray-500 font-bold uppercase mb-0.5">Reach</p>
                                                <p className="text-xs font-bold text-slate-900 dark:text-white">{post.reach || 0}</p>
                                            </div>
                                            <div className="p-2 rounded-xl bg-white dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 text-center">
                                                <p className="text-[8px] text-gray-500 font-bold uppercase mb-0.5">Engage</p>
                                                <p className="text-xs font-bold text-slate-900 dark:text-white">{post.engagement || 0}</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
                                ...
                                {/* Footer Action */}
                                <div className="p-4 bg-white/5 border-t border-gray-200 dark:border-white/5 flex items-center justify-between">
                                    ...
                                    {post.status === 'scheduled' && (
                                        <button 
                                            ...
                                            className="text-[10px] font-bold text-slate-900 dark:text-white hover:text-emerald-400 transition-colors flex items-center gap-1"
                                        >
            ...
            {showCreateModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/80 backdrop-blur-md">
                    <div className="w-full max-w-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-3xl shadow-2xl overflow-hidden">
                        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-white/5">
                            <div>
                                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Coordinate New Post</h2>
                                <p className="text-xs text-gray-500">Draft your message and set your publishing window.</p>
                            </div>
                            <button onClick={() => setShowCreateModal(false)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/5 transition-colors text-gray-500 hover:text-slate-900 dark:text-white"><X className="w-5 h-5" /></button>
                        </div>
                        <form onSubmit={handleCreatePost} className="p-6 space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <select 
                                        ...
                                        className="w-full px-4 py-3 rounded-2xl bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 appearance-none transition-all"
                                    >
                                        ...
                                        {Object.keys(platformIcons).map(p => (
                                            <button 
                                                ...
                                                className={`flex-1 p-3 rounded-2xl border transition-all flex items-center justify-center ${
                                                    formData.platform === p ? 'bg-white/10 border-white/30 text-slate-900 dark:text-white' : 'bg-white/5 border-gray-200 dark:border-white/5 text-gray-600 hover:text-gray-600 dark:text-gray-400'
                                                }`}
                                            >
                                                {platformIcons[p]}
                                            </button>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <textarea 
                                    ...
                                    className="w-full px-4 py-4 rounded-2xl bg-gray-50/50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 transition-all resize-none"
                                />
                            </div>
